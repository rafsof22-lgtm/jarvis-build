...
class KnowledgeFabricTests(unittest.TestCase):
    def setUp(self):
        ...
        self.fabric = KnowledgeFabric(Path(self.tmp.name) / "fabric.db")
    ...
    def test_invalid_coordinates_roll_back_transaction(self):
        ...
        self.assertEqual(self.fabric.db.execute("SELECT COUNT(*) FROM sources").fetchone()[0], 0)

    def test_requirement_evidence_and_no_gaps(self):
        self.fabric.upsert_requirement(
            "R1", "exact", "normalized", "control_plane",
    ...
    def test_synthesis_requires_every_message_or_reasoned_exclusion(self):
        manifest = self.fabric.ingest_source(
            pointer="p", title="t", raw=b"a",
            messages=[Message(1, "user", "a", 1, 1), Message(2, "assistant", "b", 2, 2)],
        )
        source_id = manifest["source"]["source_id"]
        identifiers = [item["message_id"] for item in manifest["messages"]]
        self.assertFalse(self.fabric.synthesis_coverage(source_id, [identifiers[0]], {})["passed"])
        self.assertTrue(
            self.fabric.synthesis_coverage(source_id, [identifiers[0]], {identifiers[1]: "duplicate summary"})["passed"]
        )
    ...
    def test_idempotent_operation_and_alert_deduplication(self):
        ...
        self.assertEqual(self.fabric.idempotent_execute("k", operation), {"ok": True})
        self.assertEqual(self.fabric.idempotent_execute("k", operation), {"ok": True})
...
class GovernanceTests(unittest.TestCase):
    ...
    def test_policy_denies_unapproved_high_risk_and_denied_tools(self):
        policy = PolicyEngine()
        self.assertEqual(
            policy.authorize(self.contract(), action="production", tool="local"),
            (False, "owner_approval_required"),
        )
        self.assertEqual(
            policy.authorize(self.contract(), action="research", tool="live"),
            (False, "tool_not_allowed"),
        )
        self.assertEqual(
            policy.authorize(self.contract(), action="production", tool="local", approval=True),
            (True, "allowed"),
        )
...
class RoutingAndIntegrationTests(unittest.TestCase):
    ...
    def test_evidence_ranker_and_classification(self):
        ...
        self.assertGreater(
            EvidenceRanker.score(authority=100, freshness=100, corroboration=100, directness=100),
            90,
        )
        with self.assertRaises(ValueError):
    ...
    def test_capability_quarantine_fails_closed(self):
        ...
        review = {
            "licence": "compatible", "security": "pass", "maintenance": "active",
            "duplication": "none", "cost": "free", "data_access": "public",
        }
    ...
    def test_signed_envelope_replay_age_and_tamper_controls(self):
        envelope = sign_envelope({"x": 1}, b"test-material", idempotency_key="i", timestamp=1000)
        self.assertTrue(verify_envelope(envelope, b"test-material", now=1001))
        self.assertFalse(verify_envelope(envelope, b"different-material", now=1001))
        self.assertFalse(verify_envelope(envelope, b"test-material", now=2000))
...
class DomainStagingTests(unittest.TestCase):
    def test_cfo_crypto_paper_agency_and_saas_slices(self):
        ...
        self.assertEqual(
            DomainStagingFactory.paper_trade(100, 10, 20, "buy", 100)["status"],
            "REJECTED_RISK_LIMIT",
        )
        self.assertEqual(DomainStagingFactory.agency_unit_economics(100, 20, 10, 2)["contribution"], 70)
        self.assertEqual(
            DomainStagingFactory.saas_stage_gate(
                tests_pass=True, security_pass=True, billing_live=False, owner_publish_approval=False,
            ),
            "INTEGRATED_STAGING",
        )
        self.assertEqual(
            DomainStagingFactory.saas_stage_gate(
                tests_pass=True, security_pass=True, billing_live=True, owner_publish_approval=False,
            ),
            "BLOCKED_PRODUCTION_APPROVAL_PATH",
        )
